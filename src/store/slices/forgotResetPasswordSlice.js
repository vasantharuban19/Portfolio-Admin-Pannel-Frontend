...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      "https://portfolio-backend-2sbb.onrender.com/api/v1/user/password/forgot",
      { email },
  ...
  (token, password, confirmPassword) => async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        `https://portfolio-backend-2sbb.onrender.com/api/v1/user/password/reset/${token}`,
...
export const clearAllForgotResetPassError = () => (dispacth) => {
  dispacth(forgotResetPassSlice.actions.clearAllErrors());
};
